"""
Основные высокоуровневые функции для инициализации, детекции
"""
import numpy as np
...
# минимальная ширина и высота прямоугольника с лицом; с этим параметром можно поэкспериментировать
# MIN_FACE_SIZE = 30	# для детекции - ОК, но слишком маленький размер для распознавания
MIN_FACE_SIZE = 45

# (первое приближение) евклидово расстояние между представлениями ниже этого порога будет означать,
# что сравниваемые лица одни и те же
...
def recognise_faces(mtcnn, model_face_recog, face_boxes: list, img: np.array, device: str = 'cpu', debug=False):
    ...
    если debug = True, то будет сохранять плохие картинки
    ...
    return face_embs
    # можно попробовать ускорить функцию обрабатывая лица в параллельных процессах,


# см. https://pytorch.org/docs/stable/notes/multiprocessing.html
...
def init_models(device: str = 'cpu'):
    """Инициализирует три модели: деткции, предобработки лиц, распознавания;
    скачивает веса (111 мб, если не скачены) нейронки для распознавания"""
...
def calc_distances_matrix(faces: list[torch.tensor]):
    """Строит верхнюю треугольную матрицу расстояний всех лицевых представлений до всех лицевых представлений"""
    n = len(faces)
